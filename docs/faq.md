...
Não necessariamente. `semtree setup` gera configuração MCP para Claude Code e Cursor. No Claude Code, o arquivo de projeto é `.mcp.json` e o servidor precisa ser aprovado antes do primeiro uso. Para Copilot e Codex, o setup grava instruções que chamam a CLI. Use `--dry-run` para revisar as mudanças antes de gravá-las.
